...
namespace MoreMechanoids
{
    public class JobGiver_Standby : ThinkNode_JobGiver
    {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            {
                IntVec3 restPos = GetRestSpot(pawn.Position, maxDanger, pawn);
        ...
        private static IntVec3 GetRestSpot(IntVec3 originCell, Danger maxDanger, Pawn me)
        {
            ...
            Danger currentDanger = me.Position.GetDangerFor(me, me.Map);
            Danger danger = (Danger) Math.Max((int) maxDanger, (int) currentDanger);

            Predicate<IntVec3> validator =
                c =>
                    c.Standable(me.Map) && me.Map.roofGrid.Roofed(c) && CoverUtility.TotalSurroundingCoverScore(c, me.Map) > 2.5f && !NextToDoor(c, me)
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                if (thing == null) break;
                IntVec3 result;
                if (CellFinder.TryFindRandomCellNear(thing.Position, me.Map, 10, validator, out result))
                {
                    return result;
                }
            }

            Predicate<IntVec3> simpleValidator =
                c => c.Standable(me.Map) && CoverUtility.TotalSurroundingCoverScore(c, me.Map) > 1 && !NextToDoor(c, me) && me.Map.reachability.CanReach(originCell, c, PathEndMode.OnCell, TraverseMode.PassDoors, danger);

            IntVec3 randomCell;
            return CellFinder.TryFindRandomCellNear(originCell, me.Map, 20, simpleValidator, out randomCell) ? randomCell : originCell;
        }

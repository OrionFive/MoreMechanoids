...
public class JobGiver_AIOpenDoors : JobGiver_DoorOpener
{
    ...
    {
        return GenClosest.ClosestThing_Global_Reachable(pawn.Position, pawn.Map, GetDoors(pawn), PathEndMode.Touch,
            TraverseParms.For(pawn), maxDist, validDoor) as Building_Door;
    }

    private static IEnumerable<Building_Door> GetDoors(Pawn pawn)
    {
        foreach (var door in pawn.Map.listerBuildings.AllBuildingsColonistOfClass<Building_Door>())
        {
            yield return door;
        }
